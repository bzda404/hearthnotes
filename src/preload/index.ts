...
const aiAPI = {
  ...
  importLocalModel: () => invoke('mt::ai::import-local-model'),
  cancelDownload: () => invoke('mt::ai::cancel-download'),
  onDownloadProgress: (handler: (progress: unknown) => void) => {
    const sub = (_e: IpcRendererEvent, progress: unknown) => handler(progress)
    ipcRenderer.on('mt::ai::download-progress', sub)
    return () => ipcRenderer.removeListener('mt::ai::download-progress', sub)
  },
...
const mcpAPI = {
  getStatus: () => invoke('mt::mcp::status'),
  setKnowledgeBase: (path: string) => invoke('mt::mcp::set-knowledge-base', path),
  clearAuditLog: () => invoke('mt::mcp::clear-audit-log'),
  sendDecision: (decision: unknown) => invoke('mt::mcp::privacy-decision', decision as never),
  onPrivacyPopup: (handler: (popup: unknown) => void) => {
    const sub = (_e: IpcRendererEvent, popup: unknown) => handler(popup)
    ipcRenderer.on('mt::mcp::privacy-popup', sub)
    return () => ipcRenderer.removeListener('mt::mcp::privacy-popup', sub)
  }
}
