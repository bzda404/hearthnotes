...
// --- imageAction helpers (refactored from monolithic method) ---

/** Determine the base directory for relative image references. */
function resolveImageBasePath (
  isTabSavedOnDisk: boolean,
  imageRelativeDirectoryBase: string,
  currentPathname: string | null,
  projectTree: unknown
): string | null {
  if (!isTabSavedOnDisk) return null
  let relativeBasePath: string | null = window.path.dirname(currentPathname!)
  if (imageRelativeDirectoryBase !== 'file' && projectTree) {
    const { pathname: rootPath } = projectTree as { pathname?: string }
...
/**
 * Handle image insertion via the 'upload' action.
 * Tries upload first, falls back to move-to-folder on failure.
 */
async function handleUploadImage (
  currentPathname: string | null,
  image: string | File,
  resolvedGlobalImageFolderPath: string
): Promise<string> {
  try {
    // currentPathname is asserted non-null — uploadImage is always called
    // when a file tab is open (currentFile.value is checked in imageAction).
    ...
    return await moveImageToFolder(
      currentPathname!,
      image,
...
/**
 * Handle image insertion via the 'folder' action.
 * Respects relative directory preference when the tab is saved on disk.
 */
async function handleFolderImage (
  isTabSavedOnDisk: boolean,
  imagePreferRelativeDirectory: boolean,
  currentPathname: string | null,
  image: string | File,
  resolvedImageRelativeFullDirectoryPath: string | null,
  resolvedGlobalImageFolderPath: string
): Promise<string> {
...
/**
 * Handle image insertion via the 'path' action.
 * String inputs are used as-is; binary inputs are saved to the image folder.
 */
async function handleLocalImage (
  isTabSavedOnDisk: boolean,
  imagePreferRelativeDirectory: boolean,
  currentPathname: string | null,
  image: string | File,
...
const imageAction = async (
  ...
  const relativeBasePath = resolveImageBasePath(
    isTabSavedOnDisk,
    imageRelativeDirectoryBase.value,
    currentPathname,
    projectTree.value
  )
  ...
  switch (imageInsertAction.value) {
    case 'upload':
      destImagePath = await handleUploadImage(currentPathname, image, resolvedGlobalImageFolderPath)
      break
    case 'folder':
      destImagePath = await handleFolderImage(
        isTabSavedOnDisk, imagePreferRelativeDirectory.value,
        currentPathname, image,
        resolvedImageRelativeFullDirectoryPath, resolvedGlobalImageFolderPath
      )
      break
    case 'path':
      destImagePath = await handleLocalImage(
        isTabSavedOnDisk, imagePreferRelativeDirectory.value,
        currentPathname, image,
        resolvedImageRelativeFullDirectoryPath, resolvedGlobalImageFolderPath
      )
