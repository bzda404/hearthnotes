...
import type { PendingPopup, PrivacyDecision, MCPServerStatus } from './mcpTypes'
...
export interface IpcInvokeChannels {
  ...
  // AI layer (Layer 2)
  'mt::ai::status': { args: []; ret: AISidecarStatus }
  ...
  'mt::ai::cancel-download': { args: []; ret: void }
  'mt::ai::is-model-present': { args: []; ret: boolean }
  'mt::ai::list-models': { args: []; ret: InstalledModelInfo[] }
  'mt::ai::load-model': { args: [modelId: string]; ret: void }
  'mt::ai::unload-model': { args: []; ret: void }
  'mt::ai::delete-model': { args: [modelId: string]; ret: void }
  'mt::ai::search-models': { args: [query: string]; ret: Array<{ name: string; desc: string; source: string }> }
  // File conversion
  'mt::fileConvert::convert': { args: [filePath: string, outputDir?: string]; ret: string }
  // MCP layer (Layer 3)
  'mt::mcp::status': { args: []; ret: MCPServerStatus }
  'mt::mcp::set-knowledge-base': { args: [path: string]; ret: void }
  'mt::mcp::clear-audit-log': { args: []; ret: MCPServerStatus }
  'mt::mcp::privacy-decision': { args: [decision: PrivacyDecision]; ret: void }
  // BM25 search layer (Phase B)
  'mt::search::bm25': {
    args: [query: string, kbName?: string]
    ret: Array<{ path: string; title: string; snippet: string; score: number }>
  }
  'mt::search::build-index': { args: [kbPath: string, kbName: string]; ret: void }
}
...
export interface IpcMainEventChannels {
  ...
  // MCP privacy popup (main → renderer)
  'mt::mcp::privacy-popup': [popup: PendingPopup]
  'mt::ai::download-progress': [progress: ModelDownloadProgress]
  'mt::ai::status-changed': [status: AISidecarStatus]
}
