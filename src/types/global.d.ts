...
declare global {
  ...
  interface AIAPI {
    getStatus(): Promise<AISidecarStatus>
  ...
  interface MCPAPI {
    ...
    setKnowledgeBase(path: string): Promise<void>
    clearAuditLog(): Promise<MCPServerStatus>
    sendDecision(decision: PrivacyDecision): Promise<void>
    onPrivacyPopup(handler: (popup: PendingPopup) => void): () => void
  }
