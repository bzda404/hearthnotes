...
class AppMenu {
   ...
   *
   * Uses dynamic state mutation instead of full menu rebuild to avoid visual
   * flicker and GC overhead. Each window menu is built once; state changes
   * are applied via getMenuItemById + per-item mutations.
   */
  updateAppMenu(recentUsedDocuments?: string[]): void {
    ...
    // Only rebuild the recently-used submenu — other items mutate in-place.
    this.windowMenus.forEach((value, key) => {
      const { menu, type } = value
      if (type !== MenuType.EDITOR || !menu) return

      // Update recently-used documents submenu
      const recentMenu = menu.getMenuItemById('recentlyUsedDocumentsMenu')
      if (recentMenu?.submenu) {
        try {
          // Rebuild the recent files submenu in-place
          const recentSubmenuItems = buildRecentDocumentsSubmenu(this._keybindings, this._preferences, recentUsedDocuments)
          const recentSubmenu = Menu.buildFromTemplate(recentSubmenuItems)
          recentMenu.submenu = recentSubmenu
        } catch { /* Ignore — Electron may reject submenu reassignment */ }
      }

      // If this menu is currently shown, refresh the app menu
      const { activeWindowId } = this
...
/**
 * Build the "Open Recent" submenu items without a full menu rebuild.
 */
function buildRecentDocumentsSubmenu(
  _keybindings: unknown,
  _preferences: unknown,
  recentUsedDocuments: string[]
): Electron.MenuItemConstructorOptions[] {
  const items: Electron.MenuItemConstructorOptions[] = []
  for (const item of recentUsedDocuments) {
    items.push({
      label: item,
      click(_menuItem, browserWindow) {
        const win = browserWindow as BrowserWindow | undefined
        if (win) {
          win.webContents.send('mt::open-file-or-folder', item)
        }
      }
    })
  }
  if (recentUsedDocuments.length > 0) {
    items.push({ type: 'separator' })
  }
  items.push({
    label: 'Clear Recently Used',
    enabled: recentUsedDocuments.length > 0,
    click() {
...
// Electron's recentDocuments submenu is handled by isOsx platform check
// in templates/file.ts; non-macOS uses a manual list. On macOS we don't
// need to rebuild the submenu — Electron manages it via the `role` property.
// So only rebuild when we're on non-macOS.
...
// This workaround is necessary because Electron's Menu.setApplicationMenu()
// sets a single global menu; per-window menus are simulated by swapping.
