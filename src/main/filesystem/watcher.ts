...
// Watcher class: manages chokidar instances for file-tree and per-file watching.
// Each editor window can register directory or file watches; the Watcher routes
// chokidar events (add/change/unlink/addDir/unlinkDir) to the appropriate
// renderer via IPC ('mt::update-object-tree' / 'mt::update-file').
//
// The watcher is instantiated by WindowManager with its preferences reference,
// and the lifecycle (watch/unwatch/close) is managed per-window.
...
const add = async(
  ...
  if (isMarkdown) {
    // Load the markdown file content and send it to the renderer.
    // The watcher manages file-sync: newly added markdown files trigger
    // content loading, while directory entries just update the tree.
    try {
...
class Watcher {
   ...
   * changed from AinCore Notes itself.
   */
