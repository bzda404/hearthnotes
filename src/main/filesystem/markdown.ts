 ...
 *
 * Uses atomic write: writes to a temporary file and renames over the target.
 * On Unix, rename() is atomic (same filesystem), so the file either contains
 * the old content or the new content — never a partially-written state.
 */
export const writeMarkdownFile = async(
  ...
  // Atomically write using tmp + rename
  const tmpPath = `${pathname}.${Math.random().toString(36).slice(2)}.tmp`
  try {
    await writeFile(tmpPath, buffer, extension, pathname)
...
export const loadMarkdownFile = async(
  ...
  if (isMixedLineEndings || isUnknownEnding || lineEnding !== 'lf') {
    ...
    // AinCore Notes always uses LF internally. If the user did not request LF line
